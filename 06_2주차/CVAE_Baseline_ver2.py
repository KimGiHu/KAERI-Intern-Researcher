...
class Decoder(nn.Module):
    ...
    def forward(self, z, c, indices1, indices2, indices3):
        ...
        h = self.unpool3(h, indices1, output_size=(h.size(0), 128, h.size(2) * 2 + 1))
...
# 모델 저장
torch.save(model.state_dict(), "./model/cvae_model_epoch15000_iter1_with_dropout_6_3.pth")
...
for i in range(len(features)):
    ...
    if features[i] == 'DV/DT':
        features[i] = 'DV_DT'
    plt.savefig('./figure/epoch15000_iter1_6_3/' + str(features[i]) + '.png', dpi=600)
