...
source_model.load_state_dict(torch.load('./model/IGBT_loss_tunning.pth'))
...
if __name__ =='__main__':
    ...
    for epoch in range(num_epochs):
        ...
        for (source_x,source_label), (target_x, target_label) in zip(dataloader_source, dataloader_target):
            ...
            loss_discriminator = nn.BCELoss()(source_mu_output, source_mu_labels) + nn.BCELoss()(target_mu_output, target_mu_labels) + \
                                 nn.BCELoss()(source_logvar_output, source_logvar_labels) + nn.BCELoss()(target_logvar_output, target_logvar_labels)
            ...
            loss_target = nn.BCELoss()(discriminator_mu(target_mu), torch.ones(target_mu.size(0), 1).to(device)) + \
                          nn.BCELoss()(discriminator_logvar(target_logvar), torch.ones(target_logvar.size(0), 1).to(device))
    ...
    torch.save(target_encoder.state_dict(), "./model/IGBT_abnormal.pth")
