...
Xnormal_DTL_norm = scaler_DTL.fit_transform(Xnormal_DTL[:524,:,:].reshape(-1, Xnormal_DTL.shape[-1])).reshape(Xnormal_DTL[:524,:,:].shape)
Xnormal_CCL_norm = scaler_CCL.fit_transform(Xnormal_CCL[:524,:,:].reshape(-1, Xnormal_CCL.shape[-1])).reshape(Xnormal_CCL[:524,:,:].shape)
Xnormal_SCL_norm = scaler_SCL.fit_transform(Xnormal_SCL[:524,:,:].reshape(-1, Xnormal_SCL.shape[-1])).reshape(Xnormal_SCL[:524,:,:].shape)

Xnormal_concat = np.concatenate( (Xnormal_RFQ_norm, Xnormal_DTL_norm, Xnormal_CCL_norm, Xnormal_SCL_norm), axis=0 )
...
for trial in range(num_trials):
    ...
    if avg_loss <= tmp :
        # 모델 저장
        tmp = avg_loss
        torch.save(model.state_dict(), "./model/mutli-module_based_cvae_best_ver4.pth")
        print("최고 모델 저장 완료!")
...
# 모델 저장
torch.save(model.state_dict(), "./model/mutli-module_based_cvae_baseline_backup_ver4.pth")
