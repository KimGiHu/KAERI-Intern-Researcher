...
data_test = Xnormal_RFQ[600:601,:,:] # visualize할 정상신호
...
optimizer = optim.Adam(model_lib.parameters(), lr=1e-5)

# 학습한 모델 불러오기
# model.load_state_dict(torch.load('./model_june_week2/mutli-module_based_cvae_best1.pth'))
model_lib.load_state_dict(torch.load('./model/mutli-module_based_cvae_loss_manual_14_waveforms.pth'))
...
with torch.no_grad():
    ...
    reconstructed, _, _ = model_lib(sample, condition)
...
for i in range(len(features)):
    ...
    plt.savefig('./figure/0624/multi-A-FLUX_manual/' + str(features[i]) + '.png', dpi=600)
