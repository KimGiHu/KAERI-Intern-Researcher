...
class Encoder(nn.Module):
    ...
    def forward(self, x, c):
        # print("input signal")
        # print(f'the shape of input : {x.shape}. the shape of module condition : {c.shape}')
        # CNN Block1
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.pool1(x)
        ...
        # print("CNN Block1")
        # print(f'the shape of output in conv1 : {x.shape}')
        # CNN Block2
        x = self.conv2(x)
        x = self.bn2(x)
        x = self.pool2(x)
        ...
        # print("CNN Block2")
        # print(f'the shape of output in conv2 : {x.shape}')
        # CNN Block3
        x = self.conv3(x)
        x = self.bn3(x)
        x = self.pool3(x)
        ...
        # print("CNN Block3")
        # print(f'the shape of output in conv3 : {x.shape}')
        # Flatten 효과
        x = x.view(x.size(0), -1) 
        # print("flatten")
        # print(f'the shape of output in flatten layer : {x.shape}')
        x = self.fc(x)

        x = torch.cat([x, c], dim=-1) 
        # print("Concatenate in Encoder")
        # print(f'the shape of concatenated output in dense layer : {x.shape}')
        ...
        # print("the mean of train data which has the size of batch")
        # print(f'the shape of mean in encoder output : {mu.shape}') 
        # print("the log-scale variance of train data which has the size of batch")
        # print(f'the shape of log-scale variance in encoder output : {logvar.shape}') 
...
for trial in range(num_trials):
    ...
    if avg_loss <= tmp :
        # 모델 저장
        tmp = avg_loss
        torch.save(model_lib.state_dict(), "./model/mutli-module_based_cvae_loss_lib_14_waveforms.pth")
        print("최고 모델 저장 완료!")
...
for trial in range(num_trials):
    ...
    if avg_loss <= tmp :
        # 모델 저장
        tmp = avg_loss
        torch.save(model_manual.state_dict(), "./model/mutli-module_based_cvae_loss_manual_14_waveforms.pth")
        print("최고 모델 저장 완료!")
