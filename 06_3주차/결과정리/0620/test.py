...
print(f'the value of standardized signal : {Xnormal_concat[524*0:524*0+1, :, 6]}') # 정규화된 신호 확인
...
print(f'the value of standardized & min-max signal : {Xnormal_concat[524*0:524*0+1, :, 6]}') # 정규화되고 나서 Min-Max 스케일링된 신호 확인
...
print(f'the value of min-max & standardized signal : {Xnormal_concat_tmp[524*0:524*0+1, :, 6]}') # Min-Max 스케일링되고 나서 정규화된 신호 확인
...
for i in range(len(features)):
    if i>=0 and i<=5:
        data_test[:,:,i] = standard_IGBT.fit_transform(data_test[:,:,i].reshape(-1, 1)).reshape(data_test[:,:,i].shape)
    if i>=6 and i<=8:
        data_test[:,:,i] = standard_FLUX.fit_transform(data_test[:,:,i].reshape(-1, 1)).reshape(data_test[:,:,i].shape)
    if i>=9 and i<=10:
        data_test[:,:,i] = standard_CAP.fit_transform(data_test[:,:,i].reshape(-1, 1)).reshape(data_test[:,:,i].shape)
    if i>=11:
        data_test[:,:,i] = standard_MOD.fit_transform((data_test[:,:,i]).reshape(-1, 1)).reshape((data_test[:,:,i]).shape)
...
model.load_state_dict(torch.load('./model/mutli-module_based_cvae_standard.pth'))
...
with torch.no_grad():
    ...
    for i in range(len(features)):
        if i>=0 and i<=5:
            sample[:,i] = standard_IGBT.inverse_transform(sample[:,i].reshape(-1, 1)).squeeze()
            reconstructed[:,i] = standard_IGBT.inverse_transform(reconstructed[:,i].reshape(-1, 1)).squeeze()
        if i>=6 and i<=8:
            sample[:,i] = standard_FLUX.inverse_transform(sample[:,i].reshape(-1, 1)).squeeze()
            reconstructed[:,i] = standard_FLUX.inverse_transform(reconstructed[:,i].reshape(-1, 1)).squeeze()
        if i>=9 and i<=10:
            sample[:,i] = standard_CAP.inverse_transform(sample[:,i].reshape(-1, 1)).squeeze()
            reconstructed[:,i] = standard_CAP.inverse_transform(reconstructed[:,i].reshape(-1, 1)).squeeze()
        if i>=11:
            sample[:,i] = standard_MOD.inverse_transform(sample[:,i].reshape(-1, 1)).squeeze()
            reconstructed[:,i] = standard_MOD.inverse_transform(reconstructed[:,i].reshape(-1, 1)).squeeze()
...
for i in range(len(features)):
    ...
    plt.savefig('./figure/wave_scale/test/' + str(features[i]) + '.png', dpi=600)
