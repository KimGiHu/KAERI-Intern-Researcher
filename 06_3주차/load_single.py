...
model.load_state_dict(torch.load('./model/mutli-module_based_cvae_only_A_FLUX.pth'))
...
with torch.no_grad():
    ...
    # 역정규화 추가
    sample = scaler_FLUX.inverse_transform(sample)
    reconstructed = scaler_FLUX.inverse_transform(reconstructed)
...
for i in range(0,1):
    ...
    plt.savefig('./figure/wave_scale/A-FLUX/' + str(features[6]) + '_abnormal_epoch500.png', dpi=600)
