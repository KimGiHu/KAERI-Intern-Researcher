...
class lstm_encoder(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=3):  # Layer 수 증가
...
model = lstm_encoder_decoder(input_size=1, hidden_size=128).to(device)  # hidden_size 증가
...
plt.savefig("./figure/before_gpt", dpi=600)
...
torch.save(model, './model/lstm_ver2_2.h5')

model_load = torch.load('./model/lstm_ver2_2.h5')
...
plt.savefig("./figure/after_gpt", dpi=600)
