...
# 오류신호 원-핫 인코딩
anomaly_labels = np.concatenate((labels1[fault_indices_RFQ[:int(len(abnormal_RFQ)*ratio)]],
...
source_model.load_state_dict(torch.load('./model/pretrained_model.pth'))
...
if __name__ =='__main__':
    ...
    for epoch in range(num_epochs):
        ...
        for (source_x,source_label), (target_x, target_label) in zip(dataloader_source, dataloader_fault):
            ...
            source_labels = torch.ones(target_z.size(0), 1).to(device)
            target_labels = torch.zeros(target_z.size(0), 1).to(device)
            loss_discriminator =  1.0*(nn.BCELoss()(discriminator(source_z[:target_z.size(0)]), source_labels) \
    ...
    torch.save(target_encoder.state_dict(), "./model/target_encoder_extended_error_ver3.pth")
