...
if __name__ =='__main__':
    ...
    for epoch in range(num_epochs):
        ...
        for x, c in dataloader1:
            ...
            for i in range(len(x)):
                x_recon, mu, logvar = source_encoder(x[i:i+1,:,:], c[i:i+1,:])
                loss = loss_function_lib(x_recon, x[i:i+1,:,:], mu, logvar)
                loss.backward()
    ...
    print("사전 모델 학습 완료!")
    torch.save(source_encoder.state_dict(), "./model/pretrained_model_ver3.pth")
