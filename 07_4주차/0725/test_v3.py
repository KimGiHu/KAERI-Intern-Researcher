...
pretrain_model.load_state_dict(torch.load('./model/pretrained_model.pth'))
...
with torch.no_grad():
    ...
    # print(x_recon.shape)
    mse_pretrain.extend((((data_test[:,:14,:] - x_recon_pretrain[:,:14,:]) ** 2)).mean(dim=(1,2)).cpu().numpy()) # 기존모델을 이용해 복원한 정상신호의 MSE
                                                                                                                     # [:,13:14,:] : (배치사이즈, 고유파형의 인덱스, 샘플 수 : 4500개)
    mse.extend((((data_test[:,:14,:] - x_recon_1[:,:14,:]) ** 2)).mean(dim=(1,2)).cpu().numpy()) # 제안된 모델을 이용해 복원한 정상신호의 MSE
...
with torch.no_grad():
    ...
    mse_error_pretrain.extend((((data_anomaly[:,:14,:] - x_pretrain_error_recon[:,:14,:]) ** 2)).mean(dim=(1,2)).cpu().numpy()) # 기존모델을 이용해 복원한 오류신호의 MSE
                                                                                                                                    # [:,13:14,:] : (배치사이즈, 고유파형의 인덱스, 샘플 수 : 4500개)
    mse_error.extend((((data_anomaly[:,:14,:] - x_error_recon_1[:,:14,:]) ** 2)).mean(dim=(1,2)).cpu().numpy()) # 제안된 모델을 이용해 복원한 오류신호의 MSE
...
for i in range(0,1):
    ...
    # sns.kdeplot((mse_pretrain), fill=True,label='pretrained Normal Loss',color='gray',cut=0) # 기존 모델을 이용해 복원한 정상신호 손실함수 분포
    ...
    plt.title(f'%s Normal vs. Fault '%args.fault)
    ...
    plt.savefig('./figure/0725/' +'%s'%args.fault + str(' Normal vs. Fault ') + '.png', dpi=600)
    print("테스트 결과 저장완료 !")
