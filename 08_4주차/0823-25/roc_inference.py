...
pretrain_model.load_state_dict(torch.load('./model/0824/pretrained_lr_schedueling_new_scaled_activation_gelu_epoch_100.pth'))

# 사전학습 모델에서 디코더 모델 불러오기
state_dict = (torch.load('./model/0824/pretrained_lr_schedueling_new_scaled_activation_gelu_epoch_100.pth'))
...
class Valid_model_Normal(nn.Module):
    ...
    def forward(self, x, c):
        self.encoder.load_state_dict(torch.load('./model/0824/adaptation_lr_scheduel_recon_activation_gelu_epoch_100.pth'))
...
plt.savefig('./figure/0824/roc/' +'%s'%args.fault + str(' Normal vs. Fault ROC Curve') + '.png', dpi=600)
