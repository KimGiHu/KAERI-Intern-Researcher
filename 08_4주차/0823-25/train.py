...
def loss_function_lib(x_recon, x, mu, logvar):    
    ...
    geo_dist = geodesic_distance(x_recon.view(x_recon.size(0),-1), x.view(x.size(0),-1)) # to calculate the Riemannian metric for the optimal distance between reconstruction signal and input signal
    KLD = -0.5 * torch.mean(torch.sum(1 + logvar - mu.pow(2) - logvar.exp(), dim=1), dim=0)# see Appendix B from VAE paper:
                                                                                           # Kingma and Welling. Auto-Encoding Variational Bayes. ICLR, 2014
    
    return MSE_lib + 1.0*KLD + geo_dist + 1e-12
...
if __name__ =='__main__':
    ...
    for epoch in range(num_epochs):
        ...
        for x, c in pbar:
            ...
            # loss = loss_function_lib(x_recon, x, mu, logvar)
            loss.backward()
    ...
    print("사전 모델 학습 완료!")
    torch.save(source_model.state_dict(), "./model/0824/pretrained_lr_schedueling_new_scaled_activation_gelu_epoch_100.pth")
