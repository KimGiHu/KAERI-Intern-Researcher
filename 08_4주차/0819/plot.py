...
pretrain_model.load_state_dict(torch.load('./model/0814/pretrained_all_waves_lr_schedueling.pth'))

# 사전학습 모델에서 디코더 모델 불러오기
state_dict = (torch.load('./model/0813/pretrained_all_waves.pth'))
...
class Valid_model_Normal(nn.Module):
    ...
    def forward(self, x, c):
        self.encoder.load_state_dict(torch.load('./model/0813/adaptation_all_waves.pth'))
...
# plt.legend()
# plt.grid(True)
# plt.savefig('./figure/0814/tsne/%s'%args.fault+' tsne_latent_space.png', dpi=600)
...
for i in range(0,1):
    ...
    plt.savefig('./figure/0814/kdeplot/' +'%s'%args.fault + str(' Normal vs. Fault kde plot') + '.png', dpi=600)
