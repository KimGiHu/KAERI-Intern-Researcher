...
pretrain_model.load_state_dict(torch.load('./model/0821/pretrained_all_waves_lr_schedueling.pth'))

# 사전학습 모델에서 디코더 모델 불러오기
state_dict = (torch.load('./model/0821/pretrained_all_waves_lr_schedueling.pth'))
...
class Valid_model_Normal(nn.Module):
    ...
    def forward(self, x, c):
        self.encoder.load_state_dict(torch.load('./model/0821/adaptation_all_waves_lr_scheduel.pth'))
...
# plt.legend()
# plt.grid(True)
# plt.savefig('./figure/0821/tsne/%s'%args.fault+' tsne_latent_space.png', dpi=600)
...
for i in range(0,1):
    ...
    # sns.kdeplot((mse), fill=True,label='Normal Loss',color='gray',cut=0) # 제안된 모델을 이용해 복원한 정상신호 손실함수 분포
    ...
    # plt.savefig('./figure/0821/kdeplot/' +'%s'%args.fault + str(' Normal vs. Fault kde plot') + '.png', dpi=600)
    ...
    plt.savefig('./figure/0821/boxplot/' +'%s'%args.fault + str(' Normal vs. Fault kde plot') + '.png', dpi=600)
