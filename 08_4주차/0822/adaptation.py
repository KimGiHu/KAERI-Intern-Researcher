...
data1 = np.concatenate( 
    (normal_RFQ[int(len(normal_RFQ)*ratio):int(len(normal_RFQ)*ratio_target),:,:],
     ...
     normal_SCL21[int(return_len(column_to_search_SCL_normal,'SCL21')*ratio):int(return_len(column_to_search_SCL_normal,'SCL21')*ratio_target),:,:]), axis=0 )
...
target_labels_concat = np.concatenate((labels1[normal_indices_RFQ[int(len(normal_RFQ)*(ratio)):int(len(normal_RFQ)*(ratio_target))]],
                                ...
                                labels4[return_indicies(column_to_search_SCL_normal, 'SCL21')[int(return_len(column_to_search_SCL_normal,'SCL21')*(ratio)):int(return_len(column_to_search_SCL_normal,'SCL21')*(ratio_target))]]), axis=0)
...
dataloader_source = DataLoader(dataset_source, batch_size=2, shuffle=True)
dataloader_target = DataLoader(dataset_target, batch_size=2, shuffle=True)
...
num_epochs = 20 # epoch 수, default : 10
...
source_model.load_state_dict(torch.load('./model/0821/pretrained_all_waves_lr_schedueling.pth'))
...
# 손실 함수 및 옵티마이저
optimizer_target = optim.Adam(target_encoder.parameters(), lr=1e-3)
optimizer_discriminator_mu = optim.Adam(discriminator_mu.parameters(), lr=1e-2)
optimizer_discriminator_logvar = optim.Adam(discriminator_logvar.parameters(), lr=1e-2)
# 학습률 스케쥴러 추가
...
if __name__ =='__main__':
    ...
    for epoch in range(num_epochs):
        ...
        for (source_x,source_label), (target_x, target_label) in zip(dataloader_source, dataloader_target):
            ...
            all_loss = loss_discriminator + loss_recon
            all_loss.backward()
        ...
        scheduler_d_mu.step(total_loss_d)
        scheduler_d_logvar.step(total_loss_d)
        scheduler_g.step(total_loss_g)

        print(f'Epoch {epoch + 1}/{num_epochs}, Loss_d: {avg_loss_d:.4f}, Loss_g: {avg_loss_g:.4f}')  

    print("정상신호 식별 네트워크 학습완료!")
    torch.save(target_encoder.state_dict(), "./model/0822/adaptation_all_waves_lr_scheduel_recon.pth")
