...
state_dict = (torch.load('./model/unique_waveforms/dvdt.pth'))
...
class Valid_model_Normal(nn.Module):
    ...
    def forward(self, x, c):
        self.encoder.load_state_dict(torch.load('./model/0807/abnormal_FLUX_C.pth'))
...
class Valid_model_Fault(nn.Module):
    ...
    def forward(self, x, c):
        self.encoder.load_state_dict(torch.load('./model/0807/abnormal_dvdt.pth'))
        self.decoder.load_state_dict(decoder_state_dict)
        mu, logvar = self.encoder(x, c)
        z = self.reparameterize(mu, logvar)
        x_recon = self.decoder(z, c)
...
pretrain_model.load_state_dict(torch.load('./model/unique_waveforms/dvdt.pth'))
...
plt.legend()
plt.grid(True)
plt.savefig('./figure/0808/%s'%args.fault+' tsne_latent_space.png', dpi=600)
exit()
# LaTeX 스타일 활성화
...
for i in range(0,1):
    ...
    plt.savefig('./figure/0807/' +'%s'%args.fault + str(' Normal vs. Fault box plot') + '.png', dpi=600)
    print("테스트 결과 저장완료 !")
