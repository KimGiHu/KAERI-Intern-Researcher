...
data1 = np.concatenate( 
    (abnormal_RFQ[:int(len(abnormal_RFQ)*ratio),:,:],
     
     abnormal_DTL3[:int(return_len(column_to_search_DTL,'DTL3')*ratio),:,:],
     abnormal_DTL5[:int(return_len(column_to_search_DTL,'DTL5')*ratio),:,:],

     abnormal_CCL1[:int(return_len(column_to_search_CCL,'CCL1')*ratio),:,:],
     abnormal_CCL2[:int(return_len(column_to_search_CCL,'CCL2')*ratio),:,:],
     abnormal_CCL3[:int(return_len(column_to_search_CCL,'CCL3')*ratio),:,:],
     abnormal_CCL4[:int(return_len(column_to_search_CCL,'CCL4')*ratio),:,:],

     abnormal_SCL1[:int(return_len(column_to_search_SCL,'SCL1')*ratio),:,:],
     abnormal_SCL5[:int(return_len(column_to_search_SCL,'SCL5')*ratio),:,:],
     abnormal_SCL9[:int(return_len(column_to_search_SCL,'SCL9')*ratio),:,:],
     abnormal_SCL12[:int(return_len(column_to_search_SCL,'SCL12')*ratio),:,:],
     abnormal_SCL14[:int(return_len(column_to_search_SCL,'SCL14')*ratio),:,:],
     abnormal_SCL15[:int(return_len(column_to_search_SCL,'SCL15')*ratio),:,:],
     abnormal_SCL18[:int(return_len(column_to_search_SCL,'SCL18')*ratio),:,:],
     abnormal_SCL21[:int(return_len(column_to_search_SCL,'SCL21')*ratio),:,:]), axis=0 )
...
target_labels_concat = np.concatenate((labels1[fault_indices_RFQ[:int(len(abnormal_RFQ)*ratio)]],
                                
                                labels2[return_indicies(column_to_search_DTL, 'DTL3')[:int(return_len(column_to_search_DTL,'DTL3')*ratio)]],
                                labels2[return_indicies(column_to_search_DTL, 'DTL5')[:int(return_len(column_to_search_DTL,'DTL5')*ratio)]], 

                                labels3[return_indicies(column_to_search_CCL, 'CCL1')[:int(return_len(column_to_search_CCL,'CCL1')*ratio)]],
                                labels3[return_indicies(column_to_search_CCL, 'CCL2')[:int(return_len(column_to_search_CCL,'CCL2')*ratio)]],
                                labels3[return_indicies(column_to_search_CCL, 'CCL3')[:int(return_len(column_to_search_CCL,'CCL3')*ratio)]],
                                labels3[return_indicies(column_to_search_CCL, 'CCL4')[:int(return_len(column_to_search_CCL,'CCL4')*ratio)]],

                                labels4[return_indicies(column_to_search_SCL, 'SCL1')[:int(return_len(column_to_search_SCL,'SCL1')*ratio)]],
                                labels4[return_indicies(column_to_search_SCL, 'SCL5')[:int(return_len(column_to_search_SCL,'SCL5')*ratio)]],
                                labels4[return_indicies(column_to_search_SCL, 'SCL9')[:int(return_len(column_to_search_SCL,'SCL9')*ratio)]],
                                labels4[return_indicies(column_to_search_SCL, 'SCL12')[:int(return_len(column_to_search_SCL,'SCL12')*ratio)]],
                                labels4[return_indicies(column_to_search_SCL, 'SCL14')[:int(return_len(column_to_search_SCL,'SCL14')*ratio)]],
                                labels4[return_indicies(column_to_search_SCL, 'SCL15')[:int(return_len(column_to_search_SCL,'SCL15')*ratio)]],
                                labels4[return_indicies(column_to_search_SCL, 'SCL18')[:int(return_len(column_to_search_SCL,'SCL18')*ratio)]],
                                labels4[return_indicies(column_to_search_SCL, 'SCL21')[:int(return_len(column_to_search_SCL,'SCL21')*ratio)]]), axis=0)
...
source_model.load_state_dict(torch.load('./model/unique_waveforms/dvdt.pth')) # 사전학습된 모델 불러오기 : 모델명마다 다 다름.
...
# 새로운 손실함수 정의
# 메트릭 텐서 정의 (예: 유클리드 공간의 경우 단위행렬)
def metric_tensor(x):
    batch_size, dim = x.size()
    return torch.eye(dim).unsqueeze(0).expand(batch_size, dim, dim).to(x.device)

# 지오데식 거리 계산
def geodesic_distance(x, y):
    min_batch_size = min(x.size(0), y.size(0))
    x = x[:min_batch_size]
    y = y[:min_batch_size]

    diff = x - y
    g = metric_tensor(x)
    dist = torch.sqrt(torch.sum(torch.bmm(diff.unsqueeze(1), torch.bmm(g, diff.unsqueeze(2))).squeeze()))
    return dist

# 새로운 손실 함수 정의
def riemannian_loss(mu1, mu2, logvar1, logvar2):
    # 두 잠재 벡터 사이의 지오데식 거리 계산
    dist_mu = geodesic_distance(mu1, mu2)
    dist_logvar = geodesic_distance(logvar1, logvar2)
    return dist_mu + dist_logvar
...
if __name__ =='__main__':
    ...
    for epoch in range(num_epochs):
        ...
        for (source_x,source_label), (target_x, target_label) in zip(dataloader_source, dataloader_target):
            ...
            loss_recon = geodesic_distance(x_recon_source.view(x_recon_source.size(0),-1), source_x.view(x_recon_source.size(0),-1)) + \
                         geodesic_distance(x_recon_target.view(x_recon_source.size(0),-1), target_x.view(x_recon_source.size(0),-1))
            ...
            loss_target = (nn.BCELoss()(discriminator_mu(target_mu), torch.ones(target_mu.size(0), 1).to(device)) + \
    ...
    print("오류신호 식별 네트워크 학습완료!")
    torch.save(target_encoder.state_dict(), "./model/0808/abnormal_dvdt.pth") # 모델 저장위치 및 이름 설정.
