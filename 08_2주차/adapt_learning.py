...
target_labels_concat = np.concatenate((labels1[normal_indices_RFQ[int(len(normal_RFQ)*(ratio)):int(len(normal_RFQ)*(ratio_target))]],
                                ...
                                labels2[return_indicies(column_to_search_DTL_normal, 'DTL3')[int(return_len(column_to_search_CCL_normal,'DTL3')*(ratio)):int(return_len(column_to_search_DTL_normal,'DTL3')*(ratio_target))]],
                                labels2[return_indicies(column_to_search_DTL_normal, 'DTL5')[int(return_len(column_to_search_CCL_normal,'DTL5')*(ratio)):int(return_len(column_to_search_DTL_normal,'DTL5')*(ratio_target))]], 
                                ...
                                labels4[return_indicies(column_to_search_SCL_normal, 'SCL1')[int(return_len(column_to_search_CCL_normal,'SCL1')*(ratio)):int(return_len(column_to_search_SCL_normal,'SCL1')*(ratio_target))]],
                                labels4[return_indicies(column_to_search_SCL_normal, 'SCL5')[int(return_len(column_to_search_CCL_normal,'SCL5')*(ratio)):int(return_len(column_to_search_SCL_normal,'SCL5')*(ratio_target))]],
                                labels4[return_indicies(column_to_search_SCL_normal, 'SCL9')[int(return_len(column_to_search_CCL_normal,'SCL9')*(ratio)):int(return_len(column_to_search_SCL_normal,'SCL9')*(ratio_target))]],
                                labels4[return_indicies(column_to_search_SCL_normal, 'SCL12')[int(return_len(column_to_search_CCL_normal,'SCL12')*(ratio)):int(return_len(column_to_search_SCL_normal,'SCL12')*(ratio_target))]],
                                labels4[return_indicies(column_to_search_SCL_normal, 'SCL14')[int(return_len(column_to_search_CCL_normal,'SCL14')*(ratio)):int(return_len(column_to_search_SCL_normal,'SCL14')*(ratio_target))]],
                                labels4[return_indicies(column_to_search_SCL_normal, 'SCL15')[int(return_len(column_to_search_CCL_normal,'SCL15')*(ratio)):int(return_len(column_to_search_SCL_normal,'SCL15')*(ratio_target))]],
                                labels4[return_indicies(column_to_search_SCL_normal, 'SCL18')[int(return_len(column_to_search_CCL_normal,'SCL18')*(ratio)):int(return_len(column_to_search_SCL_normal,'SCL18')*(ratio_target))]],
                                labels4[return_indicies(column_to_search_SCL_normal, 'SCL21')[int(return_len(column_to_search_CCL_normal,'SCL21')*(ratio)):int(return_len(column_to_search_SCL_normal,'SCL21')*(ratio_target))-1]]), axis=0)
...
source_model.load_state_dict(torch.load('./july_4th_week/model/pretrained_model_dvdt.pth'))
...
if __name__ =='__main__':
    ...
    for epoch in range(num_epochs):
        ...
        for (source_x,source_label), (target_x, target_label) in zip(dataloader_source, dataloader_target):
            ...
            loss_discriminator = 0.5*(
                                (nn.BCELoss()(discriminator(source_mu), source_mu_labels) + \
                                 nn.BCELoss()(discriminator(target_mu), target_mu_labels)) + \
                                (nn.BCELoss()(discriminator(source_logvar), source_logvar_labels) + \
                                 nn.BCELoss()(discriminator(target_logvar), target_logvar_labels)))
           
            # loss_discriminator = nn.BCELoss()(discriminator(loss_KLD(source_mu, source_logvar).reshape(1,1)),torch.ones(1,1).to(device)) + \
            #                      nn.BCELoss()(discriminator(loss_KLD(target_mu, target_logvar).reshape(1,1)),torch.zeros(1,1).to(device))
            ...
            loss_target = 0.5*(nn.BCELoss()(discriminator(target_mu), torch.ones(target_mu.size(0), 1).to(device)) + \
                          nn.BCELoss()(discriminator(target_logvar), torch.ones(target_logvar.size(0), 1).to(device)) )
    ...
    torch.save(target_encoder.state_dict(), "./july_4th_week/model/target_encoder_dvdt.pth")
