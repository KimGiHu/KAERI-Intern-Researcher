...
# SCR Fault
scr_FAULT_RFQ = np.where(np.char.find(column_to_search_RFQ, 'SCR') != -1)[0]
tmp_SCR_RFQ = len(scr_FAULT_RFQ) 

scr_FAULT_DTL = np.where(np.char.find(column_to_search_DTL, 'SCR') != -1)[0]
tmp_SCR_DTL = len(scr_FAULT_DTL)

scr_FAULT_SCL = np.where(np.char.find(column_to_search_SCL, 'SCR') != -1)[0]
tmp_SCR_SCL = len(scr_FAULT_SCL)

scr_FAULT_CCL = np.where(np.char.find(column_to_search_CCL, 'SCR') != -1)[0]
tmp_SCR_CCL = len(scr_FAULT_CCL)
...
data_anomaly = np.concatenate(
    (Xanomaly_RFQ[scr_FAULT_RFQ,:,:],
     Xanomaly_DTL[scr_FAULT_DTL,:,:],
     Xanomaly_CCL[scr_FAULT_CCL,:,:],
     Xanomaly_SCL[scr_FAULT_SCL,:,:]), axis=0)
...
anomaly_labels = np.concatenate((labels_one_hot[:tmp_SCR_RFQ,:],labels_one_hot[524*1:524*1+tmp_SCR_DTL,:],labels_one_hot[524*2:524*2+tmp_SCR_CCL,:],labels_one_hot[524*3:524*3+tmp_SCR_SCL,:]), axis=0)
...
for i in range(0,1):
    ...
    plt.hist(mse_error, bins=100, alpha=0.5, label='SCR Fault',color='red')
    sns.kdeplot((mse), fill=True,label='Normal',color='gray',cut=0)
    sns.kdeplot((mse_error), fill=True,label='SCR Fault',color='red',cut=0)
    ...
    plt.title(f'Normal vs. SCR Fault')

    plt.xscale('log')
    plt.xlim(1e-3, 1)
    plt.gca().xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'$10^{{{int(np.log10(x))}}}$'))

    plt.savefig('./figure/0715/' + str('Normal vs. SCR Fault') + '.png', dpi=600)
