...
with torch.no_grad():
    ...
    for k in range(len(data_test)):
        x_recon, _, _ = model(data_test[k:k+1,:,:], labels[k:k+1,:])
        mse.extend(((data_test[k:k+1,:,:] - x_recon) ** 2).mean(dim=(1,2)).cpu().numpy())
...
with torch.no_grad():
    ...
    for k in range(len(data_anomaly)):
        x_error_recon, _, _ = model(data_anomaly[k:k+1,:,:], anomaly_labels[k:k+1,:])
        mse_error.extend(((data_anomaly[k:k+1,:,:] - x_error_recon) ** 2).mean(dim=(1,2)).cpu().numpy())
...
for i in range(0,1):
    ...
    plt.hist(mse_error, bins=100, alpha=0.5, label='DV/DT Fault',color='red')
    sns.kdeplot((mse), fill=True,label='Normal',color='gray',cut=0)
    sns.kdeplot((mse_error), fill=True,label='DV/DT Fault',color='red',cut=0)
    ...
    plt.title(f'Normal vs. DV/DT Fault')

    plt.xscale('log')
    plt.xlim(1e-3, 1)
    plt.gca().xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'$10^{{{int(np.log10(x))}}}$'))

    plt.savefig('./figure/0715/' + str('Normal vs. DV_DT Fault') + '.png', dpi=600)
