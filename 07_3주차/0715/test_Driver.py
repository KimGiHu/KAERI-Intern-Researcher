...
driver_FAULT_RFQ = np.where(np.char.find(column_to_search_RFQ, 'Driver') != -1)[0]
tmp_driver_RFQ = len(driver_FAULT_RFQ) 

driver_FAULT_DTL = np.where(np.char.find(column_to_search_DTL, 'Driver') != -1)[0]
tmp_driver_DTL = len(driver_FAULT_DTL)

driver_FAULT_SCL = np.where(np.char.find(column_to_search_SCL, 'Driver') != -1)[0]
tmp_driver_SCL = len(driver_FAULT_SCL)

driver_FAULT_CCL = np.where(np.char.find(column_to_search_CCL, 'Driver') != -1)[0]
tmp_driver_CCL = len(driver_FAULT_CCL)
...
data_anomaly = np.concatenate(
    (Xanomaly_RFQ[driver_FAULT_RFQ,:,:],
     Xanomaly_DTL[driver_FAULT_DTL,:,:],
     Xanomaly_SCL[driver_FAULT_SCL,:,:],
     Xanomaly_CCL[driver_FAULT_CCL,:,:]), axis=0)
...
anomaly_labels = np.concatenate((labels_one_hot[:tmp_driver_RFQ,:],
                                 labels_one_hot[524*1:524*1+tmp_driver_DTL,:],
                                 labels_one_hot[524*2:524*2+tmp_driver_CCL,:],
                                 labels_one_hot[524*3:524*3+tmp_driver_SCL,:]), axis=0)
...
for i in range(0,1):
    ...
    plt.hist(mse_error, bins=100, alpha=0.5, label='Driver Fault',color='red')
    sns.kdeplot((mse), fill=True,label='Normal',color='gray',cut=0)
    sns.kdeplot((mse_error), fill=True,label='Driver Fault',color='red',cut=0)
    ...
    plt.title(f'Normal vs. Driver Fault')
    ...
    plt.savefig('./figure/0715/' + str('Normal vs. Driver Fault') + '.png', dpi=600)
