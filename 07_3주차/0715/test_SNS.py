...
# Flux Low Fault
sns_FAULT_RFQ = np.where(np.char.find(column_to_search_RFQ, 'SNS PPS') != -1)[0]
tmp_SNS_RFQ = len(sns_FAULT_RFQ) 

sns_FAULT_DTL = np.where(np.char.find(column_to_search_DTL, 'SNS PPS') != -1)[0]
tmp_SNS_DTL = len(sns_FAULT_DTL)

sns_FAULT_SCL = np.where(np.char.find(column_to_search_SCL, 'SNS PPS') != -1)[0]
tmp_SNS_SCL = len(sns_FAULT_SCL)

sns_FAULT_CCL = np.where(np.char.find(column_to_search_CCL, 'SNS PPS') != -1)[0]
tmp_SNS_CCL = len(sns_FAULT_CCL)
...
data_anomaly = np.concatenate(
    (Xanomaly_RFQ[sns_FAULT_RFQ,:,:],
     Xanomaly_DTL[sns_FAULT_DTL,:,:],
     Xanomaly_SCL[sns_FAULT_SCL,:,:],
     Xanomaly_CCL[sns_FAULT_CCL,:,:]), axis=0)
...
anomaly_labels = np.concatenate((labels_one_hot[:tmp_SNS_RFQ,:],labels_one_hot[524*1:524*1+tmp_SNS_DTL,:],labels_one_hot[524*2:524*2+tmp_SNS_CCL,:],labels_one_hot[524*3:524*3+tmp_SNS_SCL,:]), axis=0)
...
for i in range(0,1):
    ...
    plt.hist(mse_error, bins=100, alpha=0.5, label='SNS PPS Fault',color='red')
    sns.kdeplot((mse), fill=True,label='Normal',color='gray',cut=0)
    sns.kdeplot((mse_error), fill=True,label='SNS PPS Fault',color='red',cut=0)
    ...
    plt.title(f'Normal vs. SNS PPS Fault')

    plt.xscale('log')
    plt.xlim(1e-3, 1)
    plt.gca().xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f'$10^{{{int(np.log10(x))}}}$'))

    plt.savefig('./figure/0712/' + str('Normal vs. SNS PPS') + '.png', dpi=600)
