...
# Xnormal_concat = np.concatenate( (Xnormal_RFQ[:524,:,:], Xnormal_DTL[:524,:,:], Xnormal_CCL[:524,:,:], Xnormal_SCL[:524,:,:], Xnormal_RFQ[:524,:,:], Xnormal_DTL[:524,:,:], Xnormal_CCL[:524,:,:], Xnormal_SCL[:524,:,:]), axis=0 )
Xnormal_concat = np.concatenate( (Xnormal_RFQ[:524,:,:], Xnormal_RFQ[:524,:,:], Xnormal_RFQ[:524,:,:], Xnormal_RFQ[:524,:,:], Xnormal_RFQ[:524,:,:], Xnormal_RFQ[:524,:,:], Xnormal_RFQ[:524,:,:], Xnormal_RFQ[:524,:,:]), axis=0 )
...
# 원-핫 인코딩
labels_concat = np.concatenate((labels1[normal_indices_RFQ[:524]], labels1[normal_indices_DTL[:524]], labels1[normal_indices_CCL[:524]], labels1[normal_indices_SCL[:524]],labels1[normal_indices_RFQ[:524]], labels1[normal_indices_DTL[:524]], labels1[normal_indices_CCL[:524]], labels1[normal_indices_SCL[:524]]), axis=0)
...
for trial in range(num_trials):
    model_lib = CVAE(input_dim, hidden_dim, latent_dim, condition_dim).to(device)
    ...
    if avg_loss <= tmp :
        # 모델 저장
        tmp = avg_loss
        torch.save(model_lib.state_dict(), "./model/mutli-module_based_cvae_loss_lib_only_RFQ.pth")
        print("최고 모델 저장 완료!")
        
    print(f'Trial {trial + 1}/{num_trials} completed with total average loss: {total_loss_per_trial / num_epochs:.4f}')
print("라이브러리 손실함수 모델 학습 완료!")
...
for trial in range(num_trials):
    ...
    if avg_loss <= tmp :
        # 모델 저장
        tmp = avg_loss
        torch.save(model_manual.state_dict(), "./model/mutli-module_based_cvae_loss_manual_only_RFQ.pth")
        print("최고 모델 저장 완료!")
