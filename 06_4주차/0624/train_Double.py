...
# 원-핫 인코딩
labels_concat = np.concatenate((labels1[normal_indices_RFQ[:524]], labels2[normal_indices_DTL[:524]], labels3[normal_indices_CCL[:524]], labels4[normal_indices_SCL[:524]],labels1[normal_indices_RFQ[:524]], labels2[normal_indices_DTL[:524]], labels3[normal_indices_CCL[:524]], labels4[normal_indices_SCL[:524]]), axis=0)
...
for trial in range(num_trials):
    ...
    if avg_loss <= tmp :
        # 모델 저장
        tmp = avg_loss
        torch.save(model_lib.state_dict(), "./model/mutli-module_based_cvae_loss_lib_epoch15000.pth")
        print("최고 모델 저장 완료!")
...
for trial in range(num_trials):
    ...
    if avg_loss <= tmp :
        # 모델 저장
        tmp = avg_loss
        torch.save(model_manual.state_dict(), "./model/mutli-module_based_cvae_loss_manual_epoch15000.pth")
        print("최고 모델 저장 완료!")
