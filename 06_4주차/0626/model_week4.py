...
class Encoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, latent_dim, condition_dim, dropout_prob=0.2):
        ...
        self.conv1 = nn.Conv1d(input_dim, 128, kernel_size=12, padding=1)
        self.bn1 = nn.BatchNorm1d(num_features=128)
        self.pool1 = nn.MaxPool1d(kernel_size=2, stride=2)
        self.conv1_dropout = nn.Dropout(p=dropout_prob)

        self.conv2 = nn.Conv1d(128, 128, kernel_size=12, padding=1)
        self.bn2 = nn.BatchNorm1d(num_features=128)
        self.pool2 = nn.MaxPool1d(kernel_size=2, stride=2)
        self.conv2_dropout = nn.Dropout(p=dropout_prob)

        self.conv3 = nn.Conv1d(128, 128, kernel_size=12, padding=1)
        self.bn3 = nn.BatchNorm1d(num_features=128)
        self.pool3 = nn.MaxPool1d(kernel_size=2, stride=2)
        self.conv3_dropout = nn.Dropout(p=dropout_prob)

        def calc_seq_len(hidden_dim):
            tmp = (hidden_dim + 2 * 1 - 12) // 1 + 1  # conv1
            tmp = (tmp - 2) // 2 + 1           # pool1
            tmp = (tmp + 2 * 1 - 12) // 1 + 1  # conv2
            tmp = (tmp - 2) // 2 + 1           # pool2
            tmp = (tmp + 2 * 1 - 12) // 1 + 1  # conv3
            tmp = (tmp - 2) // 2 + 1           # pool3
            return tmp
...
class Decoder(nn.Module):
    def __init__(self, latent_dim, hidden_dim, output_dim, condition_dim, dropout_prob=0.2):
        ...
        self.bn1 = nn.BatchNorm1d(num_features=128)
        self.trans_conv1 = nn.ConvTranspose1d(128, 128, kernel_size=12, padding=1)
        self.conv1_dropout = nn.Dropout(p=dropout_prob)

        self.up2 = nn.Upsample(scale_factor=2)
        self.bn2 = nn.BatchNorm1d(num_features=128)
        self.trans_conv2 = nn.ConvTranspose1d(128, 128, kernel_size=12, padding=1)
        self.conv2_dropout = nn.Dropout(p=dropout_prob)
    ...
    def forward(self, z, c):
        ...
        h = self.up1(h)
        h = self.bn1(h)
        h = torch.relu(self.trans_conv1(h))
        h = self.conv1_dropout(h)

        h = self.up2(h)
        h = self.bn2(h)
        h = torch.relu(self.trans_conv2(h))
        h = self.conv2_dropout(h)

        h = self.up3(h)
        h = self.bn3(h)
        x_recon = torch.sigmoid(self.trans_conv3(h))
        # x_recon = self.conv3_dropout(x_recon)
...
class CVAE(nn.Module):
    def __init__(self, input_dim, latent_dim, hidden_dim, condition_dim, dropout_prob=0.2):
        ...
        self.encoder = Encoder(input_dim, hidden_dim, latent_dim, condition_dim, dropout_prob)
