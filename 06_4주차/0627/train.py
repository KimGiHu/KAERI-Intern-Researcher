...
def loss_function_manual(x_recon, x, mu, logvar):
    MSE_manu = torch.mean((x_recon-x)**2)
    KLD = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())# see Appendix B from VAE paper:
                                                                 # Kingma and Welling. Auto-Encoding Variational Bayes. ICLR, 2014
    return MSE_manu + 1.0*KLD + 1e-12
...
# 정상신호 N개 학습
data1 = Xnormal_concat[524*3:524*3+524,:,:] # 4가지 모듈(RFQ, DTL, CCL, SCL) 정보를 가진524 * 4 = 2096개의 데이터를 학습함. 
...
labels_one_hot = torch.tensor(labels_one_hot[524*3:524*3+524], dtype=torch.float32) # to use the output of conditional VAE
    ...
    for epoch in range(num_epochs):
        ...
        for (x, c) in dataloader1:
            ...
            for k in range(len(x)):
                ...
                x_recon_lib, mu, logvar = model_lib(x[k:k+1,:,:], c[k:k+1,:])
                # 손실함수 계산 및 backpropagation
                loss = loss_function_lib(x_recon_lib, x[k:k+1,:,:], mu, logvar)
                # loss = loss_function_manual(x_recon_lib, x, mu, logvar)
                loss.backward()
                middle_loss += loss.item()
                optimizer.step()

            total_loss += middle_loss / 16
            
            # print(f'{((x,c))}')
            # print(f'{(len(dataloader1)+1)}')
            # print(f'{x[0].shape}')
...
torch.save(model_lib.state_dict(), "./model/CVAE_0627_SCL.pth")
