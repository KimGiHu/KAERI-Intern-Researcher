...
proposed_model.load_state_dict(torch.load('./model/0826/proposed_activation_gelu.pth'))
...
with torch.no_grad():
    ...
    latent_mu_baseline_normal.append(mu_normal_baseline.cpu().numpy())
    latent_logvar_baseline_normal.append(logvar_normal_baseline.cpu().numpy())
    latent_z_baseline_normal.append(z_normal_baseline.cpu().numpy())
...
with torch.no_grad():
    ...
    # to append the abnormal mu, logvar and laten space z on baseline model
    latent_mu_baseline_abnormal.append(mu_abnormal_baseline.cpu().numpy())
    latent_logvar_baseline_abnormal.append(logvar_abnormal_baseline.cpu().numpy())
    latent_z_baseline_abnormal.append(z_anomaly_baseline.cpu().numpy())

    # to append the abnormal mu, logvar and laten space z on proposed model
...
# 배열 형태로 초기화 : normal and abnormal mu / latent space 'z'
latent_mu_baseline_normal = np.concatenate(latent_mu_baseline_normal, axis=0)
latent_mu_proposed_normal = np.concatenate(latent_mu_proposed_normal, axis=0)

latent_mu_baseline_abnormal = np.concatenate(latent_mu_baseline_abnormal, axis=0)
...
latent_mu_normal_all = np.concatenate([latent_mu_baseline_normal, latent_mu_proposed_normal], axis=0)
latent_mu_abnormal_all = np.concatenate([latent_mu_baseline_abnormal, latent_mu_proposed_abnormal], axis=0)
...
# latent_mse_pretrain_all_2d = mse_pretrain.shape(mse_pretrain.shape[0], -1)
# latent_mse_error_pretrain_all_2d = mse_pretrain.shape(mse_error_pretrain.shape[0], -1)
...
plt.legend()
plt.grid(True)
plt.savefig('./figure/0826/tsne/%s'%args.fault+' tsne_latent_space.png', dpi=600)
...
for i in range(0,1):
    ...
    plt.title(f'%s Normal vs. Fault kde plot'%args.fault)
    ...
    plt.savefig('./figure/0826/kdeplot/' +'%s'%args.fault + str(' Normal vs. Fault kde plot') + '.png', dpi=600)
    ...
    plt.savefig('./figure/0826/boxplot/' +'%s'%args.fault + str(' Normal vs. Fault box plot') + '.png', dpi=600)
